        ...
        <div className="p-4 rounded-xl border border-divider bg-subtle">
            ...
            {participants.map(p => (
              <div key={p.id} className="flex items-center gap-2 p-1.5 bg-subtle rounded-lg">
                ...
                {participants.length > 1 && <button type="button" onClick={() => handleRemovePerson(p.id)} className="text-rose-400 font-bold text-xl leading-none px-1 flex-shrink-0">&times;</button>}
              </div>
            ))}
            </div>
