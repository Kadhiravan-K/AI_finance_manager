        ...
        <div className="h-full flex flex-col">
            ...
            <div className="flex-grow overflow-y-auto p-6 space-y-3">
                ...
                sortedLists.map((note, index) => {
                    ...
                    // Fix: Added an explicit Array.isArray check to ensure type safety before calling array methods.
                    if (isChecklist && Array.isArray(note.content)) {
                        listTotal = note.content.reduce((sum, item) => sum + (item.rate || 0), 0);
                        // Fix: Added an explicit Array.isArray check to ensure type safety before calling array methods.
                        purchasedTotal = note.content.filter(i => i.isPurchased).reduce((sum, item) => sum + (item.rate || 0), 0);
                        progress = listTotal > 0 ? (purchasedTotal / listTotal) * 100 : 0;
                    }
                    ...
                    <div key={note.id} onClick={() => onSelectNote(note.id)} className="glass-card p-4 rounded-xl group flex flex-col gap-2 animate-fadeInUp cursor-pointer" style={{ animationDelay: `${index * 50}ms` }}>
                        ...
                        <div className="flex items-center gap-2 text-xs text-secondary">
                            <span>{isChecklist && Array.isArray(note.content) ? `${note.content.length} items` : 'Note'}</span>
