...
const Dashboard = ({ income, expense, isVisible }: { income: number, expense: number, isVisible: boolean }) => {
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-left mb-6 stagger-delay">
            <DashboardCard title="Income" amount={income} color="var(--color-accent-emerald)" isVisible={isVisible} style={{ '--stagger-index': 1 } as React.CSSProperties}>
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: "var(--color-accent-emerald)"}}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
            </DashboardCard>
            <DashboardCard title="Expenses" amount={expense} color="var(--color-accent-rose)" isVisible={isVisible} style={{ '--stagger-index': 2 } as React.CSSProperties}>
               <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: "var(--color-accent-rose)"}}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M18 12H6" /></svg>
            </DashboardCard>
            <DashboardCard title="Balance" amount={balance} color={balance >= 0 ? 'var(--color-text-primary)' : 'var(--color-accent-rose)'} isVisible={isVisible} style={{ '--stagger-index': 3 } as React.CSSProperties}>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 6l3 6-3 6m18-12l-3 6 3 6" /></svg>
            </DashboardCard>
        </div>
    );
}
...
const TransactionItem = ({ transaction, category, categoryPath, onEdit, onDelete, style, isVisible }: { transaction: Transaction, category: Category | undefined, categoryPath: string, onEdit: (t: Transaction) => void, onDelete: (id: string) => void, style: React.CSSProperties, isVisible: boolean }) => {
    ...
    return (
        <li style={style} className="flex flex-col glass-card p-3 rounded-xl group">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2 flex-shrink-0">
                    <span className="font-semibold" style={{ color: isIncome ? 'var(--color-accent-emerald)' : 'var(--color-accent-rose)' }}>
                        {isVisible ? `${isIncome ? '+' : '-'}${formatCurrency(transaction.amount)}` : '••••'}
                    </span>
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center">
                        {!isTransfer && (
                            <button onClick={() => onEdit(transaction)} className="p-1 text-secondary hover:text-primary" aria-label="Edit transaction">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L15.232 5.232z" /></svg>
                            </button>
                        )}
                        <button onClick={() => onDelete(transaction.id)} className="p-1 text-secondary" style={{ '--hover-color': 'var(--color-accent-rose)' } as React.CSSProperties} onMouseOver={e => e.currentTarget.style.color = 'var(--hover-color)'} onMouseOut={e => e.currentTarget.style.color = ''} aria-label="Delete transaction">
             ...
             {isSplit && totalOwed > 0 && (
                <div className="mt-2 pl-16 text-xs" style={{ color: 'var(--color-accent-yellow)' }}>
    ...
    return (
        <div>
                    ...
                    {Object.keys(groupedTransactions).map(dateKey => (
                        <div key={dateKey}>
                            <div className="sticky-header">
                               <h3 className="font-semibold text-secondary text-sm">{formatDateGroup(dateKey)}</h3>
                            </div>
                            <ul className="space-y-3 stagger-delay mt-2">
            ...
            {status === ProcessingStatus.LOADING && transactions.length === 0 && (
                <div className="text-center text-secondary p-4 animate-pulse">
