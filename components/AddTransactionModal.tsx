...
const AddTransactionModal: React.FC<AddTransactionModalProps> = ({
  ...
  useEffect(() => {
    if (isItemized) {
      setPayers([{ id: USER_SELF_ID, personName: 'You', amount: itemizedTotal, isSettled: true }]);
    } else {
       setPayers([]);
    }
          ...
          {activeTab === 'auto' ? (
            ...
            <form onSubmit={handleManualSubmit} className="flex-grow overflow-y-auto p-6 space-y-4">
              ...
              <div className="pt-4 border-t border-divider space-y-3">
                 ...
                 {isItemized && <SplitManager title="Paid By" mode={payerMode} onModeChange={(m) => setPayerMode(m as any)} participants={payers} onParticipantsChange={setPayers} totalAmount={itemizedTotal} allParticipants={allParticipants} formatCurrency={formatCurrency} isPayerManager />}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => openModal('timePicker', { initialTime: manualTime, onSave: setManualTime })} className="input-base p-2 rounded-lg w-full">{manualTime}</button>
                </div>
              </div>
