...
const ShopDetails: React.FC<Omit<ShopScreenProps, 'shops' | 'onSaveShop' | 'onDeleteShop'> & {shop: Shop}> = ({ shop, openModal, ...rest }) => {
        ...
        <div className="h-full flex flex-col">
            <div className="flex-shrink-0 p-2 overflow-x-auto border-b border-divider">
             ...
             <div className="flex-grow overflow-y-auto p-6">
                {activeTab === 'billing' && <div>POS System Here</div>}
                {activeTab === 'products' && <div>Products List Here</div>}
                {activeTab === 'invoices' && <InvoicesList shop={shop} openModal={openModal} />}
                {activeTab === 'analytics' && <div>Analytics Here</div>}
             </div>
             {activeTab === 'invoices' && (
